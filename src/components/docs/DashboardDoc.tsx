// TODO: ADD THE AZURE CONNECTOR BACK AFTER IT'S WORKING
import React from 'react';
import { 
  Box, 
  Typography, 
  Paper, 
  Grid, 
  List, 
  ListItem, 
  ListItemIcon, 
  ListItemText, 
  Divider, 
  useTheme,
  Alert,
  Stepper,
  Step,
  StepLabel 
} from '@mui/material';
import { 
  Dashboard as DashboardIcon,
  DonutLarge as DonutLargeIcon,
  ArrowForward as ArrowForwardIcon,
  CheckCircle as CheckCircleIcon,
  AddCircle as AddCircleIcon,
  Settings as SettingsIcon,
  Security as SecurityIcon,
  Cloud as CloudIcon,
  Key as KeyIcon,
  Delete as DeleteIcon,
} from '@mui/icons-material';

const DashboardDoc: React.FC = () => {
  const theme = useTheme();
  
  const connectorTypes = [
    {
      name: 'Amazon S3',
      icon: <CloudIcon sx={{ color: '#FF9900' }} />,
      description: 'Connect to AWS S3 buckets for object storage'
    },
    // {
    //   name: 'Azure Storage',
    //   icon: <CloudIcon sx={{ color: '#0078D4' }} />,
    //   description: 'Connect to Microsoft Azure Blob Storage'
    // }
  ];
  
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        <DashboardIcon sx={{ fontSize: 32, color: 'primary.main', mr: 2 }} />
        <Typography variant="h4" component="h2" fontWeight="medium" color="primary.main">
          Dashboard
        </Typography>
      </Box>
      
      <Typography variant="h6" gutterBottom sx={{ fontWeight: 'medium', mb: 2 }}>
        Your central command center for dataspace management
      </Typography>
      
      <Typography variant="body1" paragraph>
        The Dashboard provides at-a-glance visibility into your dataspace environment,
        connector status, and group memberships. From here, you can monitor your integration
        health, understand the organizational structure, and manage your storage connectors.
      </Typography>
      
      <Alert severity="info" sx={{ mb: 4 }}>
        Currently, the DLR Dataspace supports S3 connectors, with more integrations planned for future releases.
      </Alert>
      
      <Typography variant="h5" gutterBottom sx={{ mt: 4, mb: 2, fontWeight: 'medium' }}>
        Connector Management
      </Typography>
      
      <Grid container spacing={4} sx={{ mb: 4 }}>
        <Grid item xs={12} md={6}>
          <Paper 
            elevation={2} 
            sx={{ 
              p: 3, 
              height: '100%',
              background: `linear-gradient(135deg, ${theme.palette.grey[50]} 0%, ${theme.palette.grey[100]} 100%)`,
              border: `1px solid ${theme.palette.grey[200]}`,
              borderRadius: 2
            }}
          >
            <Typography variant="h6" gutterBottom sx={{ color: 'primary.main', fontWeight: 'medium', display: 'flex', alignItems: 'center' }}>
              <CheckCircleIcon sx={{ mr: 1 }} /> Connector Status
            </Typography>
            <Divider sx={{ mb: 2 }} />
            <Typography variant="body1" paragraph>
              The top section of the Dashboard displays the real-time status of your connector:
            </Typography>
            <List sx={{ pl: 2 }}>
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="Connection status indicator" 
                  secondary="Shows whether your connector is online, offline, or experiencing issues" 
                />
              </ListItem>
              {}
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="Quick action buttons" 
                  secondary="Restart connector, trigger synchronization, or access settings" 
                />
              </ListItem>
            </List>
          </Paper>
        </Grid>
        
        <Grid item xs={12} md={6}>
          <Paper 
            elevation={2} 
            sx={{ 
              p: 3, 
              height: '100%',
              background: `linear-gradient(135deg, ${theme.palette.grey[50]} 0%, ${theme.palette.grey[100]} 100%)`,
              border: `1px solid ${theme.palette.grey[200]}`,
              borderRadius: 2
            }}
          >
            <Typography variant="h6" gutterBottom sx={{ color: 'primary.main', fontWeight: 'medium', display: 'flex', alignItems: 'center' }}>
              <AddCircleIcon sx={{ mr: 1 }} /> Creating a Connector
            </Typography>
            <Divider sx={{ mb: 2 }} />
            <Typography variant="body1" paragraph>
              The connector creation process is straightforward and guided:
            </Typography>
            
            <Stepper orientation="vertical" sx={{ mb: 2 }} activeStep={4}>
              <Step>
                <StepLabel>Select connector type</StepLabel>
              </Step>
              <Step>
                <StepLabel>Configure connection details</StepLabel>
              </Step>
              <Step>
                <StepLabel>Set access permissions</StepLabel>
              </Step>
              <Step>
                <StepLabel>Test and validate connection</StepLabel>
              </Step>
            </Stepper>
            
            <Typography variant="body1">
              To create a new connector, click the <strong>"+ New Connector"</strong> button in the top-right 
              corner of the Connectors panel.
            </Typography>
          </Paper>
        </Grid>
      </Grid>
      {}
      
      <Grid container spacing={3} sx={{ mb: 4 }}>
        {connectorTypes.map((connector, index) => (
          <Grid item xs={12} sm={6} key={index}>
            <Paper 
              elevation={2} 
              sx={{ 
                p: 3,
                borderRadius: 2,
                height: '100%'
              }}
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                {connector.icon}
                <Typography variant="h6" sx={{ ml: 1, fontWeight: 'medium' }}>
                  {connector.name}
                </Typography>
              </Box>
              <Typography variant="body1">
                {connector.description}
              </Typography>
            </Paper>
          </Grid>
        ))}
      </Grid>
      
      <Paper 
        elevation={2} 
        sx={{ 
          p: 3, 
          mb: 4, 
          background: `linear-gradient(135deg, ${theme.palette.grey[50]} 0%, ${theme.palette.grey[100]} 100%)`,
          border: `1px solid ${theme.palette.grey[200]}`,
          borderRadius: 2
        }}
      >
        <Typography variant="h6" gutterBottom sx={{ color: 'primary.main', fontWeight: 'medium', display: 'flex', alignItems: 'center' }}>
          <SecurityIcon sx={{ mr: 1 }} /> Authentication & Security
        </Typography>
        <Divider sx={{ mb: 2 }} />
        <Typography variant="body1" paragraph>
          For secure access to your storage systems, you'll need to provide appropriate authentication:
        </Typography>
        
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <Paper sx={{ p: 2, bgcolor: 'background.paper' }}>
              <Typography variant="subtitle1" gutterBottom fontWeight="medium">
                <KeyIcon fontSize="small" sx={{ mr: 1, verticalAlign: 'middle', color: '#FF9900' }} />
                Amazon S3 Authentication
              </Typography>
              <List dense disablePadding>
                <ListItem sx={{ pl: 0, py: 0.5 }}>
                  <ListItemIcon sx={{ minWidth: 32 }}>
                    <ArrowForwardIcon color="primary" fontSize="small" />
                  </ListItemIcon>
                  <ListItemText primary="Access Key ID" />
                </ListItem>
                <ListItem sx={{ pl: 0, py: 0.5 }}>
                  <ListItemIcon sx={{ minWidth: 32 }}>
                    <ArrowForwardIcon color="primary" fontSize="small" />
                  </ListItemIcon>
                  <ListItemText primary="Secret Access Key" />
                </ListItem>
                <ListItem sx={{ pl: 0, py: 0.5 }}>
                  <ListItemIcon sx={{ minWidth: 32 }}>
                    <ArrowForwardIcon color="primary" fontSize="small" />
                  </ListItemIcon>
                  <ListItemText primary="Region" />
                </ListItem>
                <ListItem sx={{ pl: 0, py: 0.5 }}>
                  <ListItemIcon sx={{ minWidth: 32 }}>
                    <ArrowForwardIcon color="primary" fontSize="small" />
                  </ListItemIcon>
                  <ListItemText primary="Bucket Name" />
                </ListItem>
              </List>
            </Paper>
          </Grid>
          
          {}
        </Grid>
        
        <Typography variant="body1" sx={{ mt: 2 }}>
          <strong>Note:</strong> All credentials are encrypted and securely stored. The DLR Dataspace platform 
          never stores your raw credentials in plain text and uses industry-standard encryption methods.
        </Typography>
      </Paper>

      <Paper 
        elevation={2} 
        sx={{ 
          p: 3, 
          mb: 4, 
          background: `linear-gradient(135deg, ${theme.palette.grey[50]} 0%, ${theme.palette.grey[100]} 100%)`,
          border: `1px solid ${theme.palette.grey[200]}`,
          borderRadius: 2
        }}
      >
        <Typography variant="h6" gutterBottom sx={{ color: 'primary.main', fontWeight: 'medium', display: 'flex', alignItems: 'center' }}>
          <SettingsIcon sx={{ mr: 1 }} /> Managing Connectors
        </Typography>
        <Divider sx={{ mb: 2 }} />
        <Typography variant="body1" paragraph>
          Once created, you can manage your connectors from the Dashboard:
        </Typography>
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <List sx={{ pl: 2 }}>
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="View status and health" 
                  secondary="Monitor the connection status and synchronization health" 
                />
              </ListItem>
              {}
            </List>
          </Grid>
          <Grid item xs={12} md={6}>
            <List sx={{ pl: 2 }}>
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="Delete connector" 
                  secondary="Remove a connector when it's no longer needed" 
                />
              </ListItem>
              {}
            </List>
          </Grid>
        </Grid>
      </Paper>
      
      <Paper 
        elevation={2} 
        sx={{ 
          p: 3, 
          mb: 4, 
          background: `linear-gradient(135deg, ${theme.palette.grey[50]} 0%, ${theme.palette.grey[100]} 100%)`,
          border: `1px solid ${theme.palette.grey[200]}`,
          borderRadius: 2
        }}
      >
        <Typography variant="h6" gutterBottom sx={{ color: 'primary.main', fontWeight: 'medium', display: 'flex', alignItems: 'center' }}>
          <DeleteIcon sx={{ mr: 1 }} /> Removing a Connector
        </Typography>
        <Divider sx={{ mb: 2 }} />
        <Typography variant="body1" paragraph>
          When you no longer need a connector, you can remove it from your dataspace:
        </Typography>
        <List sx={{ pl: 2 }}>
          <ListItem sx={{ pl: 0, py: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 32 }}>
              <ArrowForwardIcon color="primary" fontSize="small" />
            </ListItemIcon>
            <ListItemText 
              primary="Find the connector card on the Connectors page" 
            />
          </ListItem>
          <ListItem sx={{ pl: 0, py: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 32 }}>
              <ArrowForwardIcon color="primary" fontSize="small" />
            </ListItemIcon>
            <ListItemText 
              primary="Click on the 'Delete' button" 
            />
          </ListItem>
          <ListItem sx={{ pl: 0, py: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 32 }}>
              <ArrowForwardIcon color="primary" fontSize="small" />
            </ListItemIcon>
            <ListItemText 
              primary="Confirm deletion when prompted" 
            />
          </ListItem>
        </List>
        <Typography variant="body1" paragraph sx={{ mt: 1 }}>
          <strong>Important:</strong> Removing a connector will:
        </Typography>
        <List sx={{ pl: 2 }}>
          <ListItem sx={{ pl: 0, py: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 32 }}>
              <ArrowForwardIcon color="error" fontSize="small" />
            </ListItemIcon>
            <ListItemText 
              primary="Remove all assets linked to this connector from the dataspace catalog" 
            />
          </ListItem>
          <ListItem sx={{ pl: 0, py: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 32 }}>
              <ArrowForwardIcon color="error" fontSize="small" />
            </ListItemIcon>
            <ListItemText 
              primary="Revoke any pending offers for assets from this connector" 
            />
          </ListItem>
          <ListItem sx={{ pl: 0, py: 0.5 }}>
            <ListItemIcon sx={{ minWidth: 32 }}>
              <ArrowForwardIcon color="error" fontSize="small" />
            </ListItemIcon>
            <ListItemText 
              primary="Delete the connection credentials from the platform (your actual storage remains untouched)" 
            />
          </ListItem>
        </List>
      </Paper>
      
      <Typography variant="h5" gutterBottom sx={{ mt: 4, mb: 2, fontWeight: 'medium' }}>
        Group Visualization & Management
      </Typography>
      
      <Grid container spacing={4} sx={{ mb: 4 }}>
        <Grid item xs={12} md={6}>
          <Paper 
            elevation={2} 
            sx={{ 
              p: 3, 
              height: '100%',
              background: `linear-gradient(135deg, ${theme.palette.grey[50]} 0%, ${theme.palette.grey[100]} 100%)`,
              border: `1px solid ${theme.palette.grey[200]}`,
              borderRadius: 2
            }}
          >
            <Typography variant="h6" gutterBottom sx={{ color: 'primary.main', fontWeight: 'medium', display: 'flex', alignItems: 'center' }}>
              <DonutLargeIcon sx={{ mr: 1 }} /> Interactive Group Visualization
            </Typography>
            <Divider sx={{ mb: 2 }} />
            <Typography variant="body1" paragraph>
              The central feature of the Dashboard is an interactive pie chart visualization:
            </Typography>
            <List sx={{ pl: 2 }}>
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="Group distribution" 
                  secondary="Visual representation of all groups in the dataspace" 
                />
              </ListItem>
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="Click-through navigation" 
                  secondary="Select any segment to view detailed information about that group" 
                />
              </ListItem>
              <ListItem sx={{ pl: 0, py: 0.5 }}>
                <ListItemIcon sx={{ minWidth: 32 }}>
                  <ArrowForwardIcon color="primary" fontSize="small" />
                </ListItemIcon>
                <ListItemText 
                  primary="Member count indicators" 
                  secondary="See how many users belong to each group at a glance" 
                />
              </ListItem>
            </List>
          </Paper>
        </Grid>
        
        {}
      </Grid>
      
      {}
      
      
      <Box sx={{ 
        p: 3, 
        bgcolor: 'primary.light', 
        color: 'primary.contrastText',
        borderRadius: 2,
        boxShadow: 1
      }}>
        <Typography variant="h6" gutterBottom fontWeight="medium">
          Pro Tip
        </Typography>
        <Typography variant="body1">
          The Dashboard serves as both your command center and connector management hub. Set up at least one connector before exploring other parts of the platform, as without an active connector, you won't be able to share assets or create offers. The visual interface makes it easy to understand the organizational structure and check your connector's health before proceeding to other areas of the platform.
        </Typography>
      </Box>
    </Box>
  );
};

export default DashboardDoc;